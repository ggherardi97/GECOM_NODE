(function () {
  ...
  function getHeaderKeysInOrder(includeHidden) {
    const keys = [];
    $("#financeGridHeader th.sortable").each(function () {
  ...
  function extractValue(row, key) {
    ...
    if (key === "movement_date") return row.movement_date;
    return row[key];
  }
  ...
  function renderTable() {
    ...
    $("#financeGridBody").html(
      bodyHtml || `<tr><td colspan="${(visibleKeys.length || 0) + 1}" class="text-muted">${esc(tt("page.finance.common.empty", "Nenhum registro encontrado."))}</td></tr>`,
    );
    $("#financeSelectedCount").text(String(state.selectedIds.size));
    $("#financeBulkActions").toggle(state.selectedIds.size > 0);
  ...
  function bindModalLookups($root, fields) {
    ...
    $root.find(".js-modal-lookup-input").each(function () {
      ...
      $menu.on("mousedown.modlookup", ".finance-lookup-item", function (event) {
        event.preventDefault();
        const id = String($(this).data("id") || "").trim();
        const label = String($(this).data("label") || "").trim();
        if (!id) return;
        $hidden.val(id);
        $input.val(label || id).removeClass("finance-invalid");
        $menu.hide();
      });
  ...
  function collectModalPayload(fields, $root) {
    ...
    fields.forEach((field) => {
      ...
      if (requiredMissing) {
        missing.push(field);
        if (field.type === "lookup") $root.find(lookupId).addClass("finance-invalid").trigger("focus");
      }
  ...
  async function quickEditSelected() {
    ...
    if (ids.length !== 1) return alert(tt("page.finance.common.selectOne", "Selecione exatamente um registro."));
    const id = ids[0];
    const row = state.rows.find((r) => String(r.id) === id);
    if (!row) return;

    window.SideModal.open({
      title: tt("page.finance.common.quickEdit", "Edicao rapida"),
  ...
  async function deleteSelected() {
    ...
    const confirmed = await (async function () {
      const result = window.confirm(tt("page.finance.common.confirmDeleteMany", "Deseja excluir os registros selecionados?"));
  ...
  function setupHeaders() {
    ...
    const filterHtml = cols
      .map(
        (col) =>
          `<th data-field="${esc(col.key)}"><input class="form-control input-sm js-filter" data-key="${esc(col.key)}" data-filter="${esc(col.key)}" placeholder="${esc(tt("page.finance.common.filter", "Filtrar"))}" /></th>`,
      )
      .join("");

    $("#financeGridHeader").html(`<th class="sv-fixed-col" data-sv-fixed="left" data-field="__select" style="width:40px;"><input id="financeSelectAll" type="checkbox"/></th>${headerHtml}`);
    $("#financeGridFilters").html(`<th class="sv-fixed-col" data-sv-fixed="left" data-field="__select"></th>${filterHtml}`);
    updateSortIndicators();
  }
  ...
  function applyFiltersFromSavedView(definition) {
    state.filters = {};
    $("#financeGridFilters .js-filter").val("");
    ...
    filters.forEach((f) => {
      ...
      $(`#financeGridFilters .js-filter[data-key="${key}"]`).val(value);
    });
  }
  ...
  function ensureSavedViewsExtraButtons() {
    const host = $("#financeSavedViewsHost");
    if (!host.length) return;

    const actions = host.find(".sv-actions").length ? host.find(".sv-actions").first() : host;
    if (!$("#btnFinanceSaveCurrentView").length) {
      actions.append(`
        <button id="btnFinanceSaveCurrentView" class="btn btn-white btn-xs sv-actionbar-btn" style="margin-left: 8px;">
          <i class="fa fa-check"></i> ${esc(tt("page.finance.common.save", "Salvar"))}
        </button>
      `);
    }

    $("#btnFinanceSaveCurrentView")
      .off("click.financeSaveCurrent")
      .on("click.financeSaveCurrent", async function (e) {
        e.preventDefault();
        e.stopPropagation();
  ...
  async function initSavedViews() {
    ...
    await window.SavedViewsGrid.init({
      entityName: String(page.entityName || `finance_${page.key || "grid"}`),
      hostSelector: "#financeSavedViewsHost",
      tableSelector: "#financeGridTable",
      headerRowSelector: "#financeGridHeader",
      filtersRowSelector: "#financeGridFilters",
  ...
  $(document).on("change", "#financeSelectAll", function () {
  ...
  $(document).on("click", "#financeGridHeader .sortable", function () {
    const key = String($(this).data("sort-key") || "");
    if (!key) return;
  ...
  $(document).on("click", "#financeGridBody tr", function (e) {
  ...
  $(document).off("click.financeCols").on("click.financeCols", "#sv_btnColumns, .sv-btn-columns, [data-action='columns']", function () {
    setTimeout(patchColumnsModalLabels, 60);
  });

  $(document).off("shown.bs.modal.financeCols").on("shown.bs.modal.financeCols", ".modal", function () {
    setTimeout(patchColumnsModalLabels, 60);
  });
  ...
  $("#btnFinanceQuickEdit").on("click", quickEditSelected);
  $("#btnFinanceDelete").on("click", deleteSelected);
  $("#btnFinanceClearSelection").on("click", function () {
    state.selectedIds.clear();
    renderTable();
  });
